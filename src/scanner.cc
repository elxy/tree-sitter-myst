...
// Determines if a character is punctuation as defined by the MyST spec.
...
extern "C" {
    void *tree_sitter_myst_external_scanner_create() {
        return new TreeSitterMyST::Scanner();
    }

    bool tree_sitter_myst_external_scanner_scan(
        void *payload,
        TSLexer *lexer,
        const bool *valid_symbols
    ) {
        TreeSitterMyST::Scanner *scanner = static_cast<TreeSitterMyST::Scanner *>(payload);
        scanner->simulate = false;
        return scanner->scan(lexer, valid_symbols);
    }

    unsigned tree_sitter_myst_external_scanner_serialize(
        void *payload,
        char* buffer
    ) {
        TreeSitterMyST::Scanner *scanner = static_cast<TreeSitterMyST::Scanner *>(payload);
    ...
    void tree_sitter_myst_external_scanner_deserialize(
        void *payload,
        char* buffer,
        unsigned length
    ) {
    ...
    void tree_sitter_myst_external_scanner_destroy(void *payload) {
        TreeSitterMyST::Scanner *scanner = static_cast<TreeSitterMyST::Scanner *>(payload);
        delete scanner;
    }
}
